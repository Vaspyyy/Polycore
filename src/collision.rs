...
use crate::{
    ...
    shape::{Shape, Health, ShapeContactCooldown, ShapeDamage, ShapeVelocity, TotalXp, Xp, XpValue},
};

pub fn check_collisions(
    mut commands: Commands,
    projectiles: Query<(Entity, &Transform), With<Projectile>>,
    mut shapes: Query<(Entity, &Transform, &mut Health, &XpValue, &mut ShapeVelocity), With<Shape>>,
    mut xp: ResMut<Xp>,
...
pub fn check_player_shape_collisions(
    mut player: Query<
        (&mut Transform, &mut Velocity, &mut PlayerHealth, &mut DamageCooldown),
        (With<Player>, Without<Shape>),
    >,
    mut shapes: Query<(&mut Transform, &mut ShapeVelocity, &ShapeDamage), (With<Shape>, Without<Player>)>,
) {
    let Ok((mut player_transform, mut player_velocity, mut player_health, mut damage_cooldown)) = player.single_mut() else { return };
    ...
    for (mut shape_transform, mut shape_velocity, shape_damage) in shapes.iter_mut() {
        ...
        let normal = if distance > 0.001 { delta / distance } else { Vec2::X };
        ...
        player_transform.translation.x = player_transform.translation.x.clamp(-player_half, player_half);
        player_transform.translation.y = player_transform.translation.y.clamp(-player_half, player_half);
        shape_transform.translation.x = shape_transform.translation.x.clamp(-shape_half, shape_half);
        shape_transform.translation.y = shape_transform.translation.y.clamp(-shape_half, shape_half);
